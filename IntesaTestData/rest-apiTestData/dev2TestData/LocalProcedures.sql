grant all on IB_DEV2_MERCHANT_OBJ.T_SHOP to IB_DEV2_TRANSACTION_OBJ;
...
CREATE OR REPLACE EDITIONABLE PROCEDURE "IB_DEV2_MERCHANT_OBJ"."CREATE_TEST_SHOPS_AUTOMATION"
...
CREATE OR REPLACE EDITIONABLE PROCEDURE "IB_DEV2_MERCHANT_OBJ"."CREATE_TEST_USERS_AUTOMATION"
    IS
    TYPE subgr_type IS VARRAY(50) OF VARCHAR(9);
    subgr_type_list subgr_type := subgr_type('100','200','300','400','001','002','003','004','005','006','007','008');
    TYPE hold_array IS VARRAY(50) OF VARCHAR(9);
...
CREATE OR REPLACE EDITIONABLE PROCEDURE "IB_DEV2_MERCHANT_OBJ"."DELETE_TEST_USERS_AUTOMATION"
...
CREATE OR REPLACE EDITIONABLE PROCEDURE "IB_DEV2_TRANSACTION_OBJ"."ADD_1_TRANSACTION_AUTOMATION" (
...
BEGIN
    ...
    SELECT AUTHORIZATION_CODE INTO autCod FROM IB_DEV2_TRANSACTION_OBJ.T_TRANSACTION order by AUTHORIZATION_CODE desc fetch first 1 row only  ;
    IF autCod is null THEN autCod := 11; END IF;
    autCod := autCod + 1;
    SELECT AREA_CODE, COMPANY_CODE, HOLDING_CODE, POS_TYPE
    INTO holdingCode, subgroupCode, groupCode, channelType FROM IB_DEV2_MERCHANT_OBJ.T_SHOP WHERE SHOP_CODE = shop_id AND ROWNUM = 1;

    IF channelType IN ('02','15') THEN channelType:= 'V'; ELSE channelType:= 'F'; END IF;

    SELECT card_type into cardBrand FROM IB_DEV2_TRANSACTION_OBJ.T_CARD_TYPE WHERE CARD_TYPE_ID = cardtype;

    INSERT INTO IB_DEV2_TRANSACTION_OBJ.T_TRANSACTION
    ...
    VALUES
    (IB_DEV2_TRANSACTION_OBJ.T_TRANSACTION_ID_SEQ.NEXTVAL,
     shop_id,
     TO_TIMESTAMP(trn_date || ' ' || trn_hour || ':' || trn_min || ':24.000000000', 'dd/mm/yy HH24:MI:SS.FF'),
     TO_DATE(trn_date, 'dd/mm/yy'),
     IB_DEV2_TRANSACTION_OBJ.GET_WEEK(TO_DATE(trn_date, 'dd/mm/yy')),
     trn_amount,
     currency,
     trn_amount,
     trn_type,
...
CREATE OR REPLACE NONEDITIONABLE PROCEDURE "IB_DEV2_TRANSACTION_OBJ"."ADD_AUTH_TRANSACTIONS_AUTOMATION"
...
--
    ...
    BEGIN
        ...
        FROM IB_DEV2_MERCHANT_OBJ.T_SHOP
    ...
    BEGIN
        ...
        INTO fi, cardType
        FROM (SELECT FI_NUMBER,DESCRIPTION  FROM IB_DEV2_TRANSACTION_OBJ.T_AUTHORIZED_FI_NUMBER JOIN IB_DEV2_TRANSACTION_OBJ.T_CARD_TYPE ON T_AUTHORIZED_FI_NUMBER.CARD_TYPE_ID =
...
BEGIN
    SELECT shop_code BULK COLLECT INTO shopsCode FROM IB_DEV2_MERCHANT_OBJ.T_SHOP
    WHERE shop_code IN
          ('110090000','110090010','110090020','110090030','110090040','110090050','110090060','110090070','110090080','110090090',
           '110090100','110090110','110090120','110090130','110090140','110090150','110090160','110090170','110090180','110090190','110090200','110090210',
           '110090220','110090230','110090240','110090250','110090260','110090270','110090280','110090290','110090300','110090310','110090320','110090330',
    ...
    FOR k in 1.. transactionPereachType LOOP
        ...
        FOR i IN 1.. shopsCode.COUNT LOOP
            ...
            INSERT INTO IB_DEV2_TRANSACTION_OBJ.T_AUTHORIZED_TRANSACTION (MESSAGE_TYPE,SHOP_CODE,TERMINAL_CODE,MERCHANT_SIA_CODE,BRANCH_SIA_CODE,POS_NO,PAN,TRANSACTION_DATE,TRANSACTION_TIME,TRANSACTION_DATETIME,
    ...
    INSERT INTO IB_DEV2_TRANSACTION_OBJ.T_AUTHORIZED_TRANSACTION (MESSAGE_TYPE,SHOP_CODE,TERMINAL_CODE,MERCHANT_SIA_CODE,BRANCH_SIA_CODE,POS_NO,PAN,TRANSACTION_DATE,TRANSACTION_TIME,TRANSACTION_DATETIME,FI_NUMBER,SIGN,
...
CREATE OR REPLACE EDITIONABLE PROCEDURE "IB_DEV2_TRANSACTION_OBJ"."ADD_TRANSACTIONS_AUTOMATION"
...
BEGIN
    ...
    FOR i IN 1 .. shop_list.count LOOP
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '29/05/19',  150.55  , 10.50  , 5.78  , 1478  , currency, card_type(1),  tr_type(1)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '29/05/19',  -20.78  , 10.50  , 5.78  , 7890  , currency, card_type(22), refound(1)   ,  01   , 01 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '29/05/19',  328.42  , 10.50  , 5.78  , 4569  , currency, card_type(15), tr_type(2)   ,  20   , 02 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '29/05/19',   78.22  , 10.50  , 5.78  , 7890  , currency, card_type(3),  tr_type(3)   ,  22   , 22 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '28/05/19', 1057.48  , 10.50  , 6.25  , 1234  , currency, card_type(1),  tr_type(1)   ,  03   , 13 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '28/05/19',  227.48  , 10.50  , 6.25  , 4596  , currency, card_type(15), tr_type(2)   ,  04   , 10 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '28/05/19',   77.48  , 10.50  , 6.25  , 2345  , currency, card_type(4),  tr_type(4)   ,  16   , 43 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '28/05/19', -227.48  , 10.50  , 6.25  , 2345  , currency, card_type(22), refound(2)   ,  11   , 10 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '27/05/19', 2024.73  , 11.50  , 5.25  , 1234  , currency, card_type(1),  tr_type(1)   ,  02   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '26/05/19',  124.73  , 11.50  , 5.25  , 1596  , currency, card_type(16), tr_type(2)   ,  05   , 50 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '25/05/19',   24.73  , 11.50  , 5.25  , 8254  , currency, card_type(5),  tr_type(5)   ,  06   , 15 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '25/05/19', -147.73  , 11.50  , 5.25  , 9735  , currency, card_type(21), refound(3)   ,  07   , 59 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '25/05/19', 1727.48  , 11.50  , 5.25  , 1973  , currency, card_type(6),  tr_type(1)   ,  08   , 33 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '24/05/19',   24.73  , 11.50  , 5.25  , 4347  , currency, card_type(7),  tr_type(6)   ,  09   , 46 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '24/05/19', 1057.48  , 10.50  , 6.25  , 1239  , currency, card_type(8),  tr_type(1)   ,  10   , 24 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '23/05/19',  124.73  , 11.50  , 5.25  , 1596  , currency, card_type(16), tr_type(2)   ,  11   , 13 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '22/05/19',  4952.96 ,  25.47 , 62.34 , 7990  , currency, card_type(6), tr_type(2)   ,  12   , 05 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '22/05/19',  -408.31 ,  3.93  , 56.43 , 6903  , currency, card_type(7), refound(3)   ,  13   , 17 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '21/05/19',  8637.69 ,  63.67 , 15.19 , 5496  , currency,  card_type(8), tr_type(1)   ,  14   , 23 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '21/05/19',  8069.13 ,  22.43 , 58.32 , 4256  , currency,  card_type(9), tr_type(10)   ,  15   , 51 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '20/05/19',  2484.47 ,  72.72 , 6.82  , 5501  , currency, card_type(8),  tr_type(2)   ,  16   , 27 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '20/05/19',  -415.14 ,  64.47 , 11.43 , 5954  , currency, card_type(10), refound(1)   ,  17   , 06 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '19/05/19',  -19.26  ,  21.57 , 35.04 , 1800  , currency, card_type(11), refound(2)   ,  18   , 34 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '18/05/19',  8837.33 ,  56.26 , 41.61 , 3010  , currency, card_type(12),  tr_type(2)   ,  19   , 26 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '18/05/19',  2390.41 ,  10.09 , 78.40 , 6543  , currency, card_type(13),  tr_type(9)   ,  20   , 50 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '18/05/19',  1892.47 ,  68.82 , 71.66 , 6697  , currency, card_type(14),  tr_type(8)   ,  21   , 37 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '17/05/19',  1755.57 ,  22.52 , 38.82 , 7830  , currency, card_type(15),  tr_type(7)   ,  22   , 48 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '17/05/19',  2388.51 ,  84.89 , 16.97 , 3670  , currency, card_type(16),  refound(2)   ,  23   , 58 );


        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '16/05/19',  3273.87 ,  86.04 , 20.01 , 3646  , currency, card_type(17),  tr_type(1)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '16/05/19',  5911.04 ,  76.99 , 16.11 , 7120  , currency, card_type(18),  tr_type(2)   ,  01   , 10 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '16/05/19',  -195.63 ,  6.31  , 69.28 , 1993  , currency, card_type(19),  refound(2)   ,  02   , 23 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '15/05/19',  1144.24 ,  17.07 , 78.01 , 9565  , currency, card_type(20),  tr_type(1)   ,  03   , 34 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '15/05/19',  -305.97 ,  52.64 , 99.82 , 9115  , currency, card_type(21),  refound(2)   ,  04   , 48 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '15/05/19',  7090.68 ,  67.11 , 49.50 , 5034  , currency, card_type(22),  tr_type(6)   ,  05   , 18 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '14/05/19',  2527.94 ,  79.23 , 14.99 , 7734  , currency,  card_type(22) ,  tr_type(2)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '14/05/19',  -1569.50,  64.98 , 96.75 , 5351  , currency,  card_type(19) ,  refound(1)   ,  00   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '13/05/19',  8247.69 ,  73.17 , 92.36 , 3449  , currency,  card_type(20),  tr_type(1)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '13/05/19',  9057.06 ,  3.32  , 37.61 , 6316  , currency,  card_type(9) ,  tr_type(11)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '13/05/19',  2274.83 ,  77.19 , 76.86 , 1903  , currency,  card_type(8) ,  tr_type(2)   ,  00   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '12/05/19',  -120.27 ,  45.03 , 66.86 , 8391  , currency,  card_type(19),  refound(1)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '12/05/19',  1713.65 ,  87.07 , 56.81 , 2353  , currency,  card_type(18),  tr_type(8)   ,  00   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '11/05/19',  -711.57 ,  12.84 , 27.35 , 4575  , currency,  card_type(17),  refound(2)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '11/05/19',  1708.72 ,  96.95 , 26.54 , 1714  , currency,  card_type(16),  tr_type(2)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '11/05/19',  9519.88 ,  61.57 , 7.36  , 1366  , currency,  card_type(15),  tr_type(7)   ,  00   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '10/05/19',  8848.03 ,  93.68 , 8.30  , 1529  , currency,  card_type(14),  tr_type(8)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '10/05/19',  1725.47 ,  97.45 , 30.62 , 9245  , currency,  card_type(15),  tr_type(7)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '10/05/19',  -736.64 ,  6.41  , 56.27 , 1025  , currency,  card_type(16),  refound(3)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '10/05/19',  3829.72 ,  89.19 , 17.07 , 8143  , currency,  card_type(17),  tr_type(1)   ,  00   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '09/05/19',  9251.79 ,  23.51 , 31.44 , 1592  , currency,  card_type(22),  tr_type(2)   ,  00   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '08/05/19',  -387.57 ,  27.31 , 99.97 , 8596  , currency,  card_type(19),  refound(2)   ,  00   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '07/05/19',  5841.08 ,  5.49  , 11.47 , 8371  , currency,  card_type(20),  tr_type(1)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '07/05/19',  -28.01  ,  55.24 , 61.43 , 9352  , currency,  card_type(1) ,  refound(2)   ,  00   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '06/05/19',  1713.65 ,  87.07 , 56.81 , 2353  , currency,  card_type(5) ,  tr_type(1)   ,  00   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '05/05/19',	7228.07  ,  36.43 , 94.36 , 3152  ,  currency,  card_type(6) ,  tr_type(2)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '05/05/19',	-232.99  ,  6.94  , 85.94 , 4360  ,  currency,  card_type(2) ,  refound(3)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '05/05/19',	486.93   ,  50.97 , 96.24 , 2008  ,  currency,  card_type(3) ,  tr_type(1)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '05/05/19',	1104.31  ,  38.38 , 46.78 , 2826  ,  currency,  card_type(4) ,  tr_type(12)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '05/05/19', 4689.77   ,  62.18 , 59.74 , 1400  ,  currency,  card_type(1) ,  tr_type(6)   ,  00   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '04/05/19', 5371.75  ,  6.62  , 83.59 , 4632  ,  currency,  card_type(10),  tr_type(1)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '04/05/19', -750.93  ,  39.19 , 48.26 , 6150  ,  currency,  card_type(11),  refound(2)   ,  00   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '03/05/19', 7559.99  ,  89.59 , 61.52 , 5184  ,  currency,  card_type(12),  tr_type(2)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '03/05/19', 4847.92  ,  54.63 , 88.32 , 8407  ,  currency,  card_type(13),  tr_type(5)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '03/05/19', 3158.21  ,  17.42 , 10.56 , 9722  ,  currency,  card_type(14),  tr_type(4)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '03/05/19', 6032.95  ,  51.16 , 84.32 , 3458  ,  currency,  card_type(15),  tr_type(3)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '03/05/19', -169.01  ,  73.57 , 61.09 , 1850  ,  currency,  card_type(16),  refound(3)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '03/05/19',  997.85  ,  29.34 , 95.93 , 8348  ,  currency,  card_type(17),  tr_type(1)   ,  00   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '01/05/19', 1775.82  ,  99.94 , 34.27 , 1821  ,  currency,  card_type(18),  tr_type(8)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '01/05/19', -677.83  ,  48.70 , 14.99 , 4021  ,  currency,  card_type(19),  refound(2)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '01/05/19', 2492.40  ,  21.99 , 96.75 , 7977  ,  currency,  card_type(22),  tr_type(1)   ,  00   , 00 );

--04 month

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '30/04/19',  328.42  , 10.50  , 5.78  , 4569  , currency, card_type(15), tr_type(2)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '30/04/19',   78.22  , 10.50  , 5.78  , 7890  , currency, card_type(3),  tr_type(3)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '30/04/19', 1057.48  , 10.50  , 6.25  , 1234  , currency, card_type(1),  tr_type(1)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '30/04/19',  227.48  , 10.50  , 6.25  , 4596  , currency, card_type(15), tr_type(2)   ,  00   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '29/04/19',  150.55  , 10.50  , 5.78  , 1478  , currency, card_type(1) ,  tr_type(1)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '29/04/19',  -20.78  , 10.50  , 5.78  , 7890  , currency, card_type(21), refound(1)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '29/04/19',  328.42  , 10.50  , 5.78  , 4569  , currency, card_type(15), tr_type(2)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '29/04/19',   78.22  , 10.50  , 5.78  , 7890  , currency, card_type(3) ,  tr_type(3)   ,  00   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '28/04/19', 1057.48  , 10.50  , 6.25  , 1234  , currency, card_type(1) ,  tr_type(1)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '28/04/19',  227.48  , 10.50  , 6.25  , 4596  , currency, card_type(15), tr_type(2)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '28/04/19',   77.48  , 10.50  , 6.25  , 2345  , currency, card_type(4) ,  tr_type(4)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '28/04/19', -227.48  , 10.50  , 6.25  , 2345  , currency, card_type(20), refound(2)   ,  00   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '27/04/19', 2024.73  , 11.50  , 5.25  , 1234  , currency, card_type(1) ,  tr_type(1)   ,  00   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '26/04/19',  124.73  , 11.50  , 5.25  , 1596  , currency, card_type(16), tr_type(2)   ,  00   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '25/04/19',   24.73  , 11.50  , 5.25  , 8254  , currency, card_type(5) ,  tr_type(5)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '25/04/19', -147.73  , 11.50  , 5.25  , 9735  , currency, card_type(19), refound(3)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '25/04/19', 1727.48  , 11.50  , 5.25  , 1973  , currency, card_type(6) ,  tr_type(1)   ,  00   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '24/04/19',   24.73  , 11.50  , 5.25  , 4347  , currency, card_type(7) ,  tr_type(6)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '24/04/19', 1057.48  , 10.50  , 6.25  , 1239  , currency, card_type(8) ,  tr_type(1)   ,  00   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '23/04/19',  124.73  , 11.50  , 5.25  , 1596  , currency, card_type(16), tr_type(2)   ,  00   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '22/04/19',  4952.96 ,  25.47 , 62.34 , 7990  , currency, card_type(6) , tr_type(2)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '22/04/19',  -408.31 ,  3.93  , 56.43 , 6903  , currency, card_type(7) , refound(3)   ,  00   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '21/04/19',  8637.69 ,  63.67 , 15.19 , 5496  , currency,  card_type(8), tr_type(1)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '21/04/19',  8069.13 ,  22.43 , 58.32 , 4256  , currency,  card_type(9), tr_type(10)   ,  00   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '20/04/19',  2484.47 ,  72.72 , 6.82  , 5501  , currency, card_type(8) ,  tr_type(2)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '20/04/19',  -415.14 ,  64.47 , 11.43 , 5954  , currency, card_type(10), refound(1)   ,  00   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '19/04/19',  -19.26  ,  21.57 , 35.04 , 1800  , currency, card_type(11), refound(2)   ,  00   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '18/04/19',  8837.33 ,  56.26 , 41.61 , 3010  , currency,  card_type(12),  tr_type(2)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '18/04/19',  2390.41 ,  10.09 , 78.40 , 6543  , currency,  card_type(13),  tr_type(9)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '18/04/19',  1892.47 ,  68.82 , 71.66 , 6697  , currency,  card_type(14),  tr_type(8)   ,  00   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '17/04/19',  1755.57 ,  22.52 , 38.82 , 7830  , currency, card_type(15),  tr_type(7)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '17/04/19',  2388.51 ,  84.89 , 16.97 , 3670  , currency, card_type(16),  refound(2)   ,  00   , 00 );


        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '16/04/19',  3273.87 ,  86.04 , 20.01 , 3646  , currency, card_type(17),  tr_type(1)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '16/04/19',  5911.04 ,  76.99 , 16.11 , 7120  , currency, card_type(18),  tr_type(2)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '16/04/19',  -195.63 ,  6.31  , 69.28 , 1993  , currency, card_type(19),  refound(2)   ,  00   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '15/04/19',  1144.24 ,  17.07 , 78.01 , 9565  , currency, card_type(20),  tr_type(1)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '15/04/19',  -305.97 ,  52.64 , 99.82 , 9115  , currency, card_type(21),  refound(2)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '15/04/19',  7090.68 ,  67.11 , 49.50 , 5034  , currency, card_type(22),  tr_type(6)   ,  00   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '14/04/19',  2527.94 ,  79.23 , 14.99 , 7734  , currency,  card_type(22) ,  tr_type(2)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '14/04/19',  -1569.50,  64.98 , 96.75 , 5351  , currency,  card_type(22) ,  refound(1)   ,  00   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '13/04/19',  8247.69 ,  73.17 , 92.36 , 3449  , currency,  card_type(20),  tr_type(1)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '13/04/19',  9057.06 ,  3.32  , 37.61 , 6316  , currency,  card_type(9) ,  tr_type(11)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '13/04/19',  2274.83 ,  77.19 , 76.86 , 1903  , currency,  card_type(8) ,  tr_type(2)   ,  00   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '12/04/19',  -120.27 ,  45.03 , 66.86 , 8391  , currency,  card_type(19),  refound(1)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '12/04/19',  1713.65 ,  87.07 , 56.81 , 2353  , currency,  card_type(11),  tr_type(8)   ,  00   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '11/04/19',  -711.57 ,  12.84 , 27.35 , 4575  , currency,  card_type(12),  refound(2)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '11/04/19',  1708.72 ,  96.95 , 26.54 , 1714  , currency,  card_type(13),  tr_type(2)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '11/04/19',  9519.88 ,  61.57 , 7.36  , 1366  , currency,  card_type(14),  tr_type(7)   ,  00   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '10/04/19',  8848.03 ,  93.68 , 8.30  , 1529  , currency,  card_type(14),  tr_type(12)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '10/04/19',  1725.47 ,  97.45 , 30.62 , 9245  , currency,  card_type(15),  tr_type(7)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '10/04/19',  -736.64 ,  6.41  , 56.27 , 1025  , currency,  card_type(16),  refound(3)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '10/04/19',  3829.72 ,  89.19 , 17.07 , 8143  , currency,  card_type(17),  tr_type(1)   ,  00   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '09/04/19',  9251.79 ,  23.51 , 31.44 , 1592  , currency,  card_type(22),  tr_type(2)   ,  00   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '08/04/19',  -387.57 ,  27.31 , 99.97 , 8596  , currency,  card_type(19),  refound(2)   ,  00   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '07/04/19',  5841.08 ,  5.49  , 11.47 , 8371  , currency,  card_type(20),  tr_type(1)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '07/04/19',  -28.01  ,  55.24 , 61.43 , 9352  , currency,  card_type(1) ,  refound(2)   ,  00   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '06/04/19',  1713.65 ,  87.07 , 56.81 , 2353  , currency,  card_type(5) ,  tr_type(1)   ,  00   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '05/04/19',	7228.07  ,  36.43 , 94.36 , 3152  ,  currency,  card_type(6) ,  tr_type(2)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '05/04/19',	-232.99  ,  6.94  , 85.94 , 4360  ,  currency,  card_type(2) ,  refound(3)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '05/04/19',	486.93   ,  50.97 , 96.24 , 2008  ,  currency,  card_type(3) ,  tr_type(1)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '05/04/19',	1104.31  ,  38.38 , 46.78 , 2826  ,  currency,  card_type(4) ,  tr_type(7)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '05/04/19', 4689.77  ,  62.18 , 59.74 , 1400  ,  currency,  card_type(1) ,  tr_type(6)   ,  00   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '04/04/19', 5371.75  ,  6.62  , 83.59 , 4632  ,  currency,  card_type(10),  tr_type(1)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '04/04/19', -750.93  ,  39.19 , 48.26 , 6150  ,  currency,  card_type(11),  refound(2)   ,  00   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '03/04/19', 7559.99  ,  89.59 , 61.52 , 5184  ,  currency,  card_type(12),  tr_type(2)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '03/04/19', 4847.92  ,  54.63 , 88.32 , 8407  ,  currency,  card_type(13),  tr_type(5)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '03/04/19', 3158.21  ,  17.42 , 10.56 , 9722  ,  currency,  card_type(14),  tr_type(4)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '03/04/19', 6032.95  ,  51.16 , 84.32 , 3458  ,  currency,  card_type(15),  tr_type(3)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '03/04/19', -169.01  ,  73.57 , 61.09 , 1850  ,  currency,  card_type(16),  refound(3)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '03/04/19',  997.85  ,  29.34 , 95.93 , 8348  ,  currency,  card_type(17),  tr_type(1)   ,  00   , 00 );

        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '01/04/19', 1775.82  ,  99.94 , 34.27 , 1821  ,  currency,  card_type(18),  tr_type(8)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '01/04/19', -677.83  ,  48.70 , 14.99 , 4021  ,  currency,  card_type(19),  refound(2)   ,  00   , 00 );
        IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (shop_list(i), '01/04/19', 2492.40  ,  21.99 , 96.75 , 7977  ,  currency,  card_type(22),  tr_type(1)   ,  00   , 00 );

    END LOOP;

-- positive increase for amount and negative for card used vs previous month shop 9006
    IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (110090060, current_month_date   , 775.82    ,  0.94 , 0.27 , 1888  ,  currency,  card_type(18),  '01'   ,  00   , 00 );
    IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (110090060, current_month_date   , 75.82     ,  0.94 , 0.27 , 1888  ,  currency,  card_type(18),  '10'   ,  00   , 00 );
    IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (110090060, current_month_date   , 5.82      ,  0.94 , 0.27 , 1118  ,  currency,  card_type(18),  '90'   ,  00   , 00 );
    IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (110090060, prev_month_date      , -100.83   ,  0.70 , 0.99 , 4021  ,  currency,  card_type(19),  '02'   ,  00   , 00 );
    IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (110090060, prev_month_date      , -10.83    ,  0.70 , 0.99 , 1118  ,  currency,  card_type(19),  '04'   ,  00   , 00 );
    IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (110090060, prev_month_date      , -10.83    ,  0.70 , 0.99 , 4021  ,  currency,  card_type(19),  '11'   ,  00   , 00 );
    IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (110090060, prev_month_date      ,  10.83    ,  0.70 , 0.99 , 4021  ,  currency,  card_type(19),  '01'   ,  00   , 00 );
    IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (110090060, prev_month_date      ,  10.83    ,  0.70 , 0.99 , 1888  ,  currency,  card_type(19),  '01'   ,  00   , 00 );

-- positive increase for card used and negative for amount vs previous month shop 9007
    IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (110090070, prev_month_date         , 775.82    ,  0.94 , 0.27 , 1888  ,  currency,  card_type(18),  '01'   ,  00   , 00 );
    IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (110090070, prev_month_date         , 75.82     ,  0.94 , 0.27 , 1888  ,  currency,  card_type(18),  '10'   ,  00   , 00 );
    IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (110090070, prev_month_date         , 5.82      ,  0.94 , 0.27 , 1118  ,  currency,  card_type(18),  '90'   ,  00   , 00 );
    IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (110090070, current_month_date      , -100.83   ,  0.70 , 0.99 , 4021  ,  currency,  card_type(19),  '02'   ,  00   , 00 );
    IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (110090070, current_month_date      , -10.83    ,  0.70 , 0.99 , 1118  ,  currency,  card_type(19),  '04'   ,  00   , 00 );
    IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (110090070, current_month_date      , -10.83    ,  0.70 , 0.99 , 4021  ,  currency,  card_type(19),  '11'   ,  00   , 00 );
    IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (110090070, current_month_date      ,  10.83    ,  0.70 , 0.99 , 3021  ,  currency,  card_type(19),  '01'   ,  00   , 00 );
    IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (110090070, current_month_date      ,  10.83    ,  0.70 , 0.99 , 1888  ,  currency,  card_type(19),  '01'   ,  00   , 00 );

-- no increase for both amount and card used vs previous month shop 9008
    IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (110090080, current_month_date   , 775.82    ,  0.94 , 0.27 , 1888  ,  currency,  card_type(18),  '01'   ,  00   , 00 );
    IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (110090080, current_month_date   , -75.82    ,  0.94 , 0.27 , 1188  ,  currency,  card_type(18),  '11'   ,  00   , 00 );
    IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (110090080, prev_month_date      , 775.82    ,  0.70 , 0.99 , 1188  ,  currency,  card_type(19),  '01'   ,  00   , 00 );
    IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION (110090080, prev_month_date      , -75.82    ,  0.70 , 0.99 , 1118  ,  currency,  card_type(19),  '11'   ,  00   , 00 );

    --add transactions for test commission account L for shop 9009
    IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION ('110090090', '31/05/19',   79.13  ,  10.22  , 58.32 , 1256  , 978,  '01', '56'  ,  00   , 01 );
    IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION ('110090090', '30/05/19',   150.52 ,   2.43  ,  8.32 , 4212  , 978,  '01', '01'  ,  00   , 10 );
    IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION ('110090090', '29/05/19',   49.49  ,    2.43 ,  8.32 , 4212  , 978,  '01', '01'  ,  00   , 10 );

    IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION ('110090090', '30/06/19',   79.13  ,  17.33  , 58.32 , 1256  , 978,  '01', '56'  ,  00   , 01 );
    IB_DEV2_TRANSACTION_OBJ.ADD_1_TRANSACTION_AUTOMATION ('110090090', '29/06/19',   250.52 ,   2.43  ,  8.32 , 4212  , 978,  '01', '01'  ,  00   , 10 );
END;
...
CREATE OR REPLACE EDITIONABLE PROCEDURE "IB_DEV2_TRANSACTION_OBJ"."DELETE_TRANSACTIONS_AUTOMATION"

    IS
BEGIN
    DELETE FROM IB_DEV2_TRANSACTION_OBJ.T_TRANSACTION WHERE shop_code IN
    ('110090000','110090010','110090020','110090030','110090040','110090050','110090060','110090070','110090080','110090090',
     '110090100','110090110','110090120','110090130','110090140','110090150','110090160','110090170','110090180','110090190','110090200','110090210',
     '110090220','110090230','110090240','110090250','110090260','110090270','110090280','110090290','110090300','110090310','110090320','110090330',
     '110090340','110090350','110090360','110090370','110090380','110090390','110090400','110090410','110090420','110090430','110090440','110090450',
     ...
     '110090940','110090950','110090960','110090970','110090980','110090990','110091000','110091010','110091020','110091030','110091040','110091050',
     '110091060','110091070','110091080','110091090','110091100','110091110','110091120','110091130','110091140','110091150','110091160','110091170');
    DELETE FROM IB_DEV2_TRANSACTION_OBJ.T_AUTHORIZED_TRANSACTION WHERE shop_code IN
     ('110090000','110090010','110090020','110090030','110090040','110090050','110090060','110090070','110090080','110090090',
     '110090100','110090110','110090120','110090130','110090140','110090150','110090160','110090170','110090180','110090190','110090200','110090210',
      '110090220','110090230','110090240','110090250','110090260','110090270','110090280','110090290','110090300','110090310','110090320','110090330',
      '110090340','110090350','110090360','110090370','110090380','110090390','110090400','110090410','110090420','110090430','110090440','110090450',
